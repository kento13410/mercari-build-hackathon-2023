...
func (r *ItemDBRepository) PutItem(ctx context.Context, item domain.Item) (domain.Item, error) {
	if _, err := r.ExecContext(ctx, "UPDATE items SET name = ?, price = ?, description = ?, category_id = ?, seller_id = ?, image = ? WHERE id = ?", item.Name, item.Price, item.Description, item.CategoryID, item.UserID, item.Image, item.ID); err != nil {
		return domain.Item{}, err
	}
